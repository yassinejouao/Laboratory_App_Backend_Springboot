...
import yass.jouao.labx.repositories.*;
...
public class AnalysisImpl implements IAnalysisService {
	...
	@Override
	public Long getAllAnalysisCount() throws NotFoundException {
		Long countAnalysis = analysisRepository.count();
...
//						.collect(Collectors.toList());
				...
				for (Test test : tests) {
					testDTOs.add(TestDTO.builder().id(test.getId()).resultTest(test.getResultTest()).result(test.getResult())
...
//			}
	...
	public List<AnalysisResultDTO> getAnalysisRapport(Long analysisTypeId, LocalDateTime startDate,
		...
		List<Analysis> analyses = analysisRepository.findAllByStartDateBetween(startDate, endDate);
